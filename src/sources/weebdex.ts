...
async function fetchManga(ctx: SearchContext): Promise<SourceMangaOutput> {
  ...
  if (manga.length === 0) throw new NotFoundError("[WeebDex] no manga found");
...
async function fetchPages(ctx: ChapterContext): Promise<SourcePagesOutput> {
  const chapterData: ChapterPagesResponse = await ctx.proxiedFetcher(
    ctx.chapter.url,
    {
      headers: {
        Origin: "https://weebdex.org",
  ...
  if (!chapterData || !chapterData.node) {
    throw new NotFoundError(
      `[WeebDex] Invalid chapter data for ${ctx.chapter.id}`,
    );
  }
