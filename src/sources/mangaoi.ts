...
function parseManga(html: string, url: string): Manga {
  ...
  const altTitlesText = $(
    'header.ep-cover_ch .manga-info .item-head:contains("Alternative:")',
  )
    .next(".item-content")
    .text()
    .trim();
  ...
  let coverUrl =
    $("header.ep-cover_ch .comic-cover").css("background-image") || undefined;
  if (coverUrl !== undefined)
    coverUrl = coverUrl.replace(/^url\(["']?/, "").replace(/["']?\)$/, "");

  if (!coverUrl) {
    coverUrl =
      $("header.ep-cover_ch .novel-cover img.image-novel").attr("src") ||
      undefined;
  }
  ...
  const statusText = $(
    'header.ep-cover_ch .manga-info .item-head:contains("Status")',
  )
    .next(".item-name")
    .text()
    .trim()
...
async function fetchManga(ctx: SearchContext): Promise<SourceMangaOutput> {
  ...
  //     });
  // });
  const mangaUrl = `${baseUrl}/read-manga/${
    ctx.titleInput
      .toLowerCase()
