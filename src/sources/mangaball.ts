...
import type { MangaContext, ChapterContext, SearchContext, ScrapeContext } from '@/utils/context';
...
async function getSession(ctx: ScrapeContext) {
    ...
    if (!phpSession) throw new Error('[MangaBall] Failed to get PHPSESSID cookie')
...
async function fetchPages(ctx: ChapterContext): Promise<SourcePagesOutput> {
    ...
    const html = typeof response.text === 'function' ? await response.text() : response;
    ...
    if (!found || pages.length === 0) {
        throw new NotFoundError(`[MangaBall] No pages found for chapter: ${ctx.chapter.url}`);
    }
