...
async function fetchManga(ctx: SearchContext): Promise<SourceMangaOutput> {
  ...
  if (!url)
    throw new NotFoundError(`[MangaKatana] can't find ${ctx.titleInput}`);
  if (url.endsWith("/fc")) {
    url = url.slice(0, -3);
  }
...
async function fetchChapters(ctx: MangaContext): Promise<SourceChaptersOutput> {
  ...
  const $ = cheerio.load(response);
  const chapters = $(".uk-table tbody tr")
    .map((i, el) => {
      const $row = $(el);
      ...
      const regex =
        /^(Chapter|Epilogue|Prologue|Special)?\s*([\d\.]+)?\:?\s*(.*)$/i;
...
async function fetchPages(ctx: ChapterContext): Promise<SourcePagesOutput> {
  ...
  const targetScript = scripts.find(
    (script) =>
      $(script).html()!.includes("var ytaw") &&
      $(script).html()!.includes("var thzq"),
  );

  const scriptText = $(targetScript).html() || "";

  try {
    const ast = acorn.parse(scriptText, {
      ecmaVersion: 2020,
      sourceType: "script",
    });
    ...
    if (!thzqArray)
      throw new Error("[MangaKatana] thzq array not found in script");
