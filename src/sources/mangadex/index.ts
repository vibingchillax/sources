...
async function fetchManga(ctx: SearchContext): Promise<SourceMangaOutput> {
  ...
  if (!search.data)
    throw new NotFoundError(
      `[MangaDex] No search results for ${ctx.titleInput}`,
    );
...
async function fetchChapters(ctx: MangaContext): Promise<SourceChaptersOutput> {
  ...
  async function safeFetch(
    ...
    try {
      ...
      if (retries > 0) {
        console.warn(
          `[MangaDex] fetch failed at offset=${offset}, retrying... (${3 - retries + 1})`,
        );
  ...
  if (failed) {
    console.warn(
      `[MangaDex] returning partial results (${chapters.length} chapters)`,
    );
  }
