...
async function fetchManga(ctx: SearchContext): Promise<SourceMangaOutput> {
    ...
    const response: Response = await ctx.proxiedFetcher(`${baseUrl}/wp-admin/admin-ajax.php`, {
        body: formData,
        method: "POST",
        // headers: {
        //     "X-Origin": baseUrl,
    ...
    if (!response.series[0].all.length) throw new Error(`[Armageddon] error while connecting to api`);
...
function getChapters($: cheerio.CheerioAPI): Chapter[] {
    ...
    return chapterItems.map((li, index) => {
        const $li = $(li);
        const $a = $li.find('a');
