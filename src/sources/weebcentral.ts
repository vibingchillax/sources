...
function parseManga(html: string): Manga[] {
  ...
  $("article.bg-base-300").each((_, el) => {
    ...
    const yearText = root
      .find('section:nth-child(2) div:contains("Year:") span')
      .text()
      .trim();
    const year = yearText ? Number(yearText) : null;

    const statusText = root
      .find('section:nth-child(2) div:contains("Status:") span')
      .text()
      .trim()
      .toLowerCase();
    ...
    root
      .find('section:nth-child(2) div:contains("Author(s):") a')
      .each((_, a) => {
    ...
    const tags: string[] = [];
    root
      .find('section:nth-child(2) div:contains("Tag(s):") span')
      .each((_, span) => {
    ...
    const coverUrl = root
      .find("section:first-child a article picture img")
      .attr("src");
...
async function fetchChapters(ctx: MangaContext): Promise<SourceChaptersOutput> {
  const response = await ctx.proxiedFetcher(
    ctx.manga.url + "/full-chapter-list",
  );
